...
class FlightSearchTool(BaseTool):
    ...
    args_schema: Type[BaseModel] = FlightSearchInput  # BaseModel타입인데 실제 클래스는 FlightSearchInput이다라는 뜻
...
class NearbyPlacesTool(BaseTool):
    ...
    def find_nearby_places(self, location, radius):
        ...
        params = {
            ...
            "language": "ko",
            "key": os.getenv("GOOGLE_API_KEY")
        } #  "type": "tourist_attraction" --> 관광, 여행목적으로 넣어두었다.
        response = requests.get(url, params=params).json()
...
class ExchangeRateInput(BaseModel):
    from_currency: str = Field(..., description="원본 통화의 코드 (예: USD)") # ... 가변매개변수의 개념
